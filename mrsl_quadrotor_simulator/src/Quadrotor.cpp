...
Quadrotor::Wrench Quadrotor::update(Quadrotor::MotorState des_motor_rpm,
                                    double dt) {
  for (int i = 0; i < 4; i++) {
  ...
  Quadrotor::MotorState motor_rpm_dot =
      (des_motor_rpm - curr_motor_rpm_) / motor_time_constant_;
  ...
  output(3) = km_ * (motor_rpm_sq(0) + motor_rpm_sq(1) - motor_rpm_sq(2) -
                     motor_rpm_sq(3));
  return output;
}
